...
class SingleScaleModel(BaseModel):
    ...
    def __init__(self, opt):
        super(SingleScaleModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        # l2 loss
        l_l2_hc_hr = self.l2_pix(hc_hr_recon, self.gt)
        l_l2_hc_lr = self.l2_pix(hc_lr_recon, self.gt)
        l_l2_lc_hr = self.l2_pix(lc_hr_recon, self.lr_gt)
        l_l2_lc_lr = self.l2_pix(lc_lr_recon, self.lr_gt)
        l_total += l_l2_hc_hr
        l_total += l_l2_lc_hr
        l_total += l_l2_hc_lr
        l_total += l_l2_lc_lr
        ...
        if current_iter >= self.opt["train"].get("start_gan_opt_iter", 0):
            # gan loss
        ...
        if self.ema_decay > 0:
            if self.opt['train'].get("ada_decay", 0) > 0:
                decay=min(self.ema_decay, (1+current_iter)/(self.opt['train']['ada_decay'] + current_iter))
            else:
