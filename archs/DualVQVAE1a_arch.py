...
class MultiScaleEncoder(nn.Module):
    ...
    def forward(self, input, output_scales=None):
        outputs = []
...
class DualVQVAE1a_ARCH(nn.Module):
    ...
    def __init__(self, in_channel=3, scale_factor=4,quant_params=[[4,512],[8,256]], code_book_opt=True,encoder_init_consistent=False):
        super(DualVQVAE1a_ARCH, self).__init__()
        self.max_depth=3
        self.scale_factor = scale_factor
        self.lr_encoder = MultiScaleEncoder(in_channel, max_depth=3,need_swinlayer=True, output_scale=[i[0] for i in quant_params])
        self.hr_encoder = MultiScaleEncoder(in_channel, max_depth=3,need_swinlayer=False, output_scale=[i[0] for i in quant_params])
    ...
    def forward(self, lr, hr=None, rtn_embed_sort=False, multi_scale=False,output_scales=None):
        if not multi_scale:
            encs_hr = self.hr_encoder(hr)
            encs_lr = self.lr_encoder(torch.nn.functional.interpolate(lr, scale_factor=self.scale_factor))
        else:
            ms_encs_hr = self.hr_encoder(hr, output_scales=output_scales)
            ms_encs_lr = self.lr_encoder(torch.nn.functional.interpolate(lr, scale_factor=self.scale_factor), output_scales=output_scales)
            encs_hr=[ms_encs_hr[-1]]
            encs_lr=[ms_encs_lr[-1]]
        ...
        for i in range(self.max_depth)[::-1]:
            if 2 ** (i + 1) in self.quantize_scales:
                ...
                quant_losses_hc_hr.append(quant_loss_hc_hr)
                quant_losses_hc_lr.append(quant_loss_hc_lr)
                ...
                if multi_scale:
                    return decs_hc_hr[-1], decs_hc_lr[-1], decs_lc_hr[-1], decs_lc_lr[-1], \
                        ...
                        enc_constraints[0], ms_encs_hr, ms_encs_lr
